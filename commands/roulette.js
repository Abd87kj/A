...
const { generateWheelImage, generateWheelGif, generateBannerImage, generateSmoothAngles, getFrameDelays, getTotalGifDuration, SPIN_STEPS } = require('../utils/wheelCanvas');
    ...
    try {
      if (guildSettings.bannerUrl) {
        bannerMsg = await interaction.reply({
          content: guildSettings.bannerUrl,
      ...
      if (timeLeft <= 0) {
        ...
        if (s.players.length < 2) {
          ...
          try {
            ...
            await m.edit({ content: '❌ انتهى الوقت! ما انضم كافي لاعبين.' });
          } catch (e) {}
          try {
  ...
  async handleButton(interaction, client) {
    ...
    if (interaction.customId === 'roulette_join') {
      ...
      try {
        ...
        await m.edit({ content: buildStatusText(session, session.timeLeft ?? JOIN_TIME) });
      } catch (e) {}
...
async function startSpin(channel, session, channelId) {
  ...
  try {
    const sm = await channel.messages.fetch(session.messageId);
    await sm.delete();
  } catch (e) {}
  ...
  let gifBuf;
  try {
    gifBuf = await generateWheelGif(session.players, winnerIdx, session.colors);
  } catch (e) {
  ...
  // نرسل GIF - الفريم الأخير فيه صورة الفائز في المركز
  const att = new AttachmentBuilder(gifBuf, { name: 'wheel.gif' });
  await channel.send({ files: [att] });

  // ننتظر المدة الحقيقية الكاملة للـ GIF (بدل رقم تقريبي ثابت)
  // + هامش بسيط عشان نعطي وقت لديسكورد يرفع/يعرض الصورة عند المستخدمين
  const totalGifTime = getTotalGifDuration(SPIN_STEPS, 3000) + 600;
  await new Promise(r => setTimeout(r, totalGifTime));

  await channel.send({ content: `🏆 **وقفت على ${winner.displayName}!**` });
