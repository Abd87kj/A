...
module.exports = {
  ...
  async handleButton(interaction, client) {
    ...
    if (['elim_random', 'elim_hacker', 'elim_nuke', 'elim_shield'].includes(interaction.customId)) {
      const s = getSession(channelId);
      if (!s || s.status !== 'elimination') {
        return interaction.reply({ content: '⚠️ مو وقت الطرد الحين.', ephemeral: true });
      }
...
function buildEliminationButtons(players, spinnerId) {
  ...
  // صف أزرار القوى الخاصة (نفس ستايل شفاف/رمادي)
...
async function startSpin(channel, session, channelId) {
  ...
  const elimMsg = await channel.send({
    content: `🎯 **وقفت على ${winner.displayName}!**\n<@${winner.id}> اختر شخص تطرده أو استخدم إحدى القوى الخاصة خلال **30 ثانية**:`,
...
// ──────────────────────────────────────────────
//  زر: تهكير (طرد لاعبين عشوائياً)
// ──────────────────────────────────────────────

async function handleHackerElimination(channel, session, channelId) {
  await channel.send({ content: `💻 **${session.currentSpinner.displayName}** استخدم التهكير! سيتم طرد لاعبين عشوائياً.` });
...
// ──────────────────────────────────────────────
//  زر: نيوك (طرد كل اللاعبين)
// ──────────────────────────────────────────────

async function handleNukeElimination(channel, session, channelId) {
  const spinner = session.currentSpinner;
  await channel.send({ content: `☢️ **${spinner.displayName}** استخدم النيوك! تم طرد جميع اللاعبين الآخرين دفعة واحدة!` });
...
async function handleSelfShield(channel, session, channelId) {
  const spinner = session.currentSpinner;
  const u = getUser(spinner.id, session.guildId);
  u.activeShields = (u.activeShields || 0) + 1;
  saveUser(u);

  await channel.send({ content: `🛡️ **${spinner.displayName}** حمى نفسه! حصل على درع إضافي ولن يتم طرد أحد هذه الجولة.` });
