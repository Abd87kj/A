...
module.exports = {
  ...
  async handleButton(interaction, client) {
    ...
    // ── زر الطرد العشوائي (مجاني للجميع، بدون الحاجة لشراء أي قطعة) ──
...
function buildEliminationButtons(players, spinnerId) {
  ...
  // ملاحظة: 🎲 طرد عشوائي مجاني للجميع، بقية الأزرار تتطلب امتلاك القطعة من المتجر
  const powerRow = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('elim_random').setLabel('🎲 طرد عشوائي (مجاني)').setStyle(ButtonStyle.Secondary),
...
async function startSpin(channel, session, channelId) {
  ...
  const elimMsg = await channel.send({
    content: `🎯 **وقفت على ${winner.displayName}!**\n<@${winner.id}> اختر شخص تطرده، أو استخدم الطرد العشوائي 🎲 (مجاني)، أو إحدى القوى الخاصة (تحتاج تكون شاريها من المتجر 🛒) خلال **30 ثانية**:`,
...
// ──────────────────────────────────────────────
//  زر: طرد عشوائي (مجاني للجميع — لا يحتاج شراء أو نقاط)
// ──────────────────────────────────────────────
...
async function handleRandomElimination(channel, session, channelId) {
  ...
  await channel.send({ content: `🎲 **${session.currentSpinner.displayName}** استخدم الطرد العشوائي المجاني!` });
