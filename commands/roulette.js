...
const { generateWheelGif, generateBannerImage, getTotalGifDuration, getSpinSteps } = require('../utils/wheelCanvas');
...
const ELIMINATION_TIMEOUT = 30000; // 30 ثانية للاختيار
...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const session = {
      ...
      currentSpinner: null,       // اللاعب اللي دور عليه العجلة (سيختار من يطرد)
...
function buildEliminationButtons(players, spinnerId) {
  ...
  const rows = [];
  // Discord يسمح 5 أزرار في صف، وأقصى 5 صفوف
...
async function startSpin(channel, session, channelId) {
  ...
  const waitMsg = await channel.send({ content: '⏳ **جاري تحضير العجلة...**' });

  let gifBuf;
  try {
    gifBuf = await generateWheelGif(session.players, winnerIdx, session.colors, session.spinConfig);
  } catch (e) {
    console.error('خطأ في توليد GIF:', e);
  ...
  const att = new AttachmentBuilder(gifBuf, { name: 'wheel.gif' });
  await channel.send({ files: [att] });

  const steps = getSpinSteps(session.spinConfig?.spinDuration ?? 8);
  const totalGifTime = getTotalGifDuration(steps, 3000) + 600;
  await new Promise(r => setTimeout(r, totalGifTime));

  // ── إذا بقي لاعبان فقط: الفائز الأخير ──
  ...
  // مؤقت تلقائي إذا ما اختار
  session._eliminationTimer = setTimeout(async () => {
    const current = getSession(channelId);
...
async function handleElimination(channel, session, channelId, target, targetIdx) {
  // إلغاء المؤقت التلقائي إن وجد
  if (session._eliminationTimer) {
    clearTimeout(session._eliminationTimer);
    session._eliminationTimer = null;
  }

  // إزالة أزرار الطرد
  try {
    const elimMsg = await channel.messages.fetch(session.eliminationMessageId);
  ...
  if (targetUser.activeShields > 0) {
    ...
    // اللعبة تستمر بدون طرد، دور جديد
    session.status = 'waiting_next';
    session.eliminationPhase = false;
...
async function continueGame(channel, session, channelId) {
  ...
  await channel.send({ content: `🎰 **جولة جديدة!** ${s.players.map(p => `**${p.displayName}**`).join(' • ')}` });

  // إعادة ضبط البانر المسج مؤقتاً (مش ضروري نرسله مرة ثانية)
  // ندير دوران مباشرة
...
async function announceChampion(channel, session, channelId, champion) {
  ...
  const resultEmbed = new EmbedBuilder()
    ...
    .addFields(
      {
        name: '💸 المطرودون بالترتيب',
        value: eliminatedList,
      },
      {
        name: '🏆 الفائز',
        value: `👑 **${champion.displayName}** +${pot}`,
      }
    )
    .setTimestamp();
