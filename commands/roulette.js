...
const { generateWheelImage, generateBannerImage, generateSpinGif } = require('../utils/wheelCanvas');
...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    try {
      const { buffer } = await generateWheelImage(session.players, null, 0, colors);
      const wheelAtt = new AttachmentBuilder(buffer, { name: 'wheel.png' });
    ...
    const timer = setInterval(async () => {
      ...
      try {
        ...
        const { buffer } = await generateWheelImage(current.players, null, 0, current.colors);
        const wheelAtt = new AttachmentBuilder(buffer, { name: 'wheel.png' });
  ...
  async handleButton(interaction, client) {
    ...
    if (interaction.customId === 'roulette_join') {
      ...
      setSession(channelId, session);
      try {
        const { buffer } = await generateWheelImage(session.players, null, 0, session.colors);
        const wheelAtt = new AttachmentBuilder(buffer, { name: 'wheel.png' });
...
async function startSpin(channel, session, channelId) {
  ...
  // نحذف رسالة العجلة الثابتة ونرسل GIF واحد سلس
  try {
  ...
  let waitMsg;
  try {
    waitMsg = await channel.send('🎰 **العجلة تدور... جهّز نفسك!**');
  } catch (e) {}

  let gifBuffer;
  try {
    gifBuffer = await generateSpinGif(session.players, winnerIdx, session.colors, 60);
  } catch (e) {
    console.error('فشل توليد GIF العجلة:', e);
    deleteSession(channelId);
    return;
  }
  ...
  try {
    ...
    await channel.send({ content: '🎰 **العجلة تدور...**', files: [att] });
  } catch (e) {}

  // مدة الدوران التقريبية (نفس مدة الفريمات + توقف ثانيتين على النتيجة)
  const approxSpinMs = 4500;
  await new Promise(r => setTimeout(r, approxSpinMs));
