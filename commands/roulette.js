...
const BANNER_BUFFER = require('../assets/bannerBuffer');
...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const session = {
      ...
      messageId: null,
      bannerMsgId: null,
      timeLeft: JOIN_TIME, // FIX: track real remaining time on the session
    };
    ...
    let bannerMsg;
    try {
      const bannerAtt = new AttachmentBuilder(BANNER_BUFFER, { name: 'banner.png' });
      ...
      console.error('Banner error:', e); // FIX: log full error, not just message
      bannerMsg = await interaction.reply({
        content: '🎰 **روليت الحظ**',
        components: buildLobbyButtons(),
        fetchReply: true,
    ...
    try {
      ...
      console.error('Wheel image error:', e); // FIX: surface the real failure instead of silently falling back
    ...
    const timer = setInterval(async () => {
      ...
      s.timeLeft = timeLeft; // FIX: keep session in sync so join/leave handlers show correct time
      ...
      try {
        ...
        console.error('Wheel update error:', e);
      }
    }, 5000);
  },
  ...
  async handleButton(interaction, client) {
    ...
    if (interaction.customId === 'roulette_join') {
      if (!session || session.status !== 'waiting') return interaction.reply({ content: '⚠ اللعبة مو في وقت الانضمام.', ephemeral: true });
      ...
      try {
        ...
        await m.edit({ content: buildStatusText(session, session.timeLeft ?? JOIN_TIME), files: [wheelAtt] }); // FIX: real remaining time, not hardcoded 30
      } catch (e) {
        console.error('Join wheel update error:', e);
      }
...
function buildPlayer(interaction) {
  return {
    ...
    // FIX: was `interaction.user.avatarURL.bind(interaction.user)` — that stores a *function*,
    // not a usable image URL, so the canvas image loader fails (silently, due to the empty catches).
...
async function startSpin(channel, session, channelId) {
  ...
  for (let i = 0; i < STEPS; i++) {
    ...
    const delay = i < 4 ? 200 : i < 7 ? 300 : i < 9 ? 500 : i < 11 ? 700 : 900;
    ...
    try {
      ...
      console.error('Spin frame error:', e);
    }
  }
