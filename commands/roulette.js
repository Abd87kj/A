...
// تخزين المؤقتات خارج الـ session
...
async function startSpin(channel, session, channelId) {
  ...
  // إزالة أزرار اللوبي فقط في الجولة الأولى
  ...
  // ✅ إرسال الـ GIF وانتظار انتهائه قبل أي شيء
  ...
  const totalGifTime = getTotalGifDuration(steps, 5000) + 1000;
  await new Promise(r => setTimeout(r, totalGifTime));

  // بعد ما توقف العجلة - تحقق إذا بقي لاعب وحيد
  if (session.players.length === 1) {
    await announceChampion(channel, session, channelId, winner);
    return;
  }

  // ✅ مرحلة الطرد بعد توقف العجلة
