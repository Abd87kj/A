...
try {
 ...
 console.log('✅ تم تحميل صورة البانر:', BANNER_BUFFER.length, 'bytes');
} catch (e) {
 console.error('❌ ما قدر يحمّل صورة البانر:', e.message);
}
...
module.exports = {
 ...
 async execute(interaction, client) {
   ...
   let bannerMsg;
   try {
     if (!BANNER_BUFFER) throw new Error('البانر ما محمّل');
 ...
 async handleButton(interaction, client) {
   ...
   if (interaction.customId === 'roulette_join') {
     ...
     try {
       ...
       await m.edit({ content: buildStatusText(session, 30), files: [wheelAtt] });
     } catch (e) {}
