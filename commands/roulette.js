...
module.exports = {
  ...
  async handleButton(interaction, client) {
    ...
    if (interaction.customId.startsWith('eliminate_')) {
      // نجيب الـ session من جديد عشان نضمن أحدث نسخة
      const s = getSession(channelId);
      ...
      // نعطي رد فوري للـ interaction عشان ما تنتهي صلاحيتها
      await interaction.deferUpdate();

      // نشيل أزرار الطرد من الرسالة
      try {
...
async function startSpin(channel, session, channelId) {
  ...
  // ── توليد صورة العجلة بزاوية عشوائية (تبدو وكأنها وقفت بشكل طبيعي) ──
  ...
  // نضيف offset عشوائي صغير داخل شريحة الفائز (يخلي العجلة تبدو طبيعية)
  const sliceAngle = (Math.PI * 2) / session.players.length;
  const randomOffset = (Math.random() - 0.5) * sliceAngle * 0.6; // ±30% من الشريحة
  ...
  // انتظار بسيط (1.5 ثانية) بعد الصورة
  await new Promise(r => setTimeout(r, 1500));

  // بعد العجلة - تحقق إذا بقي لاعب وحيد
...
async function announceChampion(channel, session, channelId, champion) {
  ...
  // ── بنر الفائز المخصص (إن وجد) ──
  if (session.winnerBannerUrl) {
    try {
      await channel.send({ content: session.winnerBannerUrl });
    } catch (e) {}
  }
  ...
  try {
    ...
    winnerImageBuf = await generateWinnerImage(champion, pot, session.bet);
  } catch (e) {
