...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === 'ثيم') {
      ...
      const themeNames = { default: 'الافتراضي 🌌', gold: 'ذهبي 🥇', red: 'أحمر 🔴', emerald: 'زمردي 💚', purple: 'بنفسجي 💜' };
    ...
    // ══════════════════════════════════════════════════════
    if (sub === 'شكل') {
      const nوع = interaction.options.getString('نوع');
      saveGuildSettings(guildId, { wheelStyle: nوع });
      await interaction.deferReply({ ephemeral: true });

      const settings = getGuildSettings(guildId);
      const colors = getThemeColors(settings);
      const styleConfig = WHEEL_STYLES[nوع];
      ...
      const styleNames = { classic: 'الكلاسيكي 🎡', neon: 'نيون 💜', minimal: 'مينيمال ⬜', royal: 'ملكي 👑' };
      const embed = new EmbedBuilder()
        .setTitle(`✅ تم تفعيل شكل: ${styleNames[nوع]}`)
