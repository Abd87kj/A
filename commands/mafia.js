...
async function handleNightAction(interaction, session, channelId, client) {
  ...
  if (id.startsWith('mafia_night_kill_')) {
    ...
    // عطّل الأزرار في الـ DM وأكد الاختيار
    await interaction.update({
      embeds: [new EmbedBuilder()
        .setColor(0x8b0000)
        .setTitle('✅ تم اختيار الضحية')
        .setDescription(`اخترت **${target?.displayName}** للقتل.\n\nانتظر نتيجة الليلة...`)
      ],
      components: [],
    });

    // أعلن للشات العام إن المافيا انتهت (بدون اسم)
    if (channel) {
      await channel.send({
        embeds: [new EmbedBuilder()
          .setColor(0x8b0000)
  ...
  if (id.startsWith('mafia_night_save_')) {
    ...
    // عطّل الأزرار في الـ DM
    await interaction.update({
      embeds: [new EmbedBuilder()
        .setColor(0x006400)
        .setTitle('✅ تم اختيار الحماية')
        .setDescription(`اخترت حماية **${target?.displayName}**.\n\nانتظر نتيجة الليلة...`)
      ],
      components: [],
    });
  ...
  if (id.startsWith('mafia_night_investigate_')) {
    ...
    // عطّل الأزرار في الـ DM وأظهر النتيجة للمحقق بس
    await interaction.update({
      embeds: [new EmbedBuilder()
        .setColor(0x00008b)
        .setTitle('🔍 نتيجة التحقيق')
    ...
    // أعلن للشات العام إن المحقق انتهى (بدون نتيجة أو اسم)
    if (channel) {
      await channel.send({
        embeds: [new EmbedBuilder()
          .setColor(0x00008b)
          .setTitle('🔵 المحقق انتهى ✅')
          .setDescription('قام المحقق بالتحقيق من أحد اللاعبين.')
        ],
      });
    }
