  ...
  async handleButton(interaction, client) {
    ...
    if (id === 'mafia_reveal_role') {
      ...
      try {
        ...
        return interaction.reply({ content: '⚠️ صار خطأ أثناء تحميل بطاقتك.', ephemeral: true });
      }
    }
...
// ──────────────────────────────────────────────
//  إرسال بطاقات الأدوار — زر "اعرف دورك" ephemeral
//  كل لاعب يضغط الزر ويشوف دوره بس هو (Only you can see this)
// ──────────────────────────────────────────────
async function sendRoleCards(session, client, channel) {
  ...
  await channel.send({
    content: '🎴 **تم توزيع الأدوار!** اضغط الزر أدناه لتعرف دورك — بس أنت تشوفه!',
    components: [row],
  });
}
