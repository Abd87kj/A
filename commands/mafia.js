...
const { generateRoleCard, generateResultImage } = require('../utils/mafiaCanvas');
...
async function sendRoleCards(session, client, channel) {
  for (const player of session.players) {
    try {
      ...
      const cardBuf = await generateRoleCard(player.role, player.displayName, avatarUrl);
...
async function endGame(channel, session, channelId, winnerTeam) {
  ...
  try {
    ...
    resultBuf = await generateResultImage(winnersWithAv, losersWithAv, winnerTeam);
  } catch (e) {}
