...
function buildPuzzleEmbed(session) {
  const alive = session.players.filter((p) => !p.eliminated);
  return new EmbedBuilder()
    .setColor(theme.colors.danger)
    .setTitle(`${theme.emojis.bomb} لعبة البومب — الجولة ${session.round}`)
    .setDescription(
      `**كلمة تبدأ بالحرف الأول وتنتهي بالثاني:**\n\n` +
      `### \`${session.puzzle.first} - ${session.puzzle.last}\`\n\n` +
      `${theme.emojis.fire} القنبلة عند: <@${session.holderId}>\n` +
      `${theme.emojis.timer} الوقت المتبقي: ${session.roundSeconds} ثانية`
    )
    .addFields({
      name: 'اللاعبون الباقون',
      value: alive.map((p) => `<@${p.id}>`).join(' • ') || 'لا يوجد'
    })
    .setFooter({ text: 'اكتب كلمة صحيحة قبل انفجار القنبلة!' });
}
...
async function runRound(interaction, session) {
  ...
  const roundMsg = await channel.send({ embeds: [buildPuzzleEmbed(session)] });
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription('اضغط **انضمام للعبة** للمشاركة.\nمنشئ اللعبة يضغط **ابدأ اللعبة** عند الجهوزية (٢ لاعبين فأكثر).')
  ...
  async handleButton(interaction) {
    ...
    if (id === 'bomb_join') {
      ...
      session.players.push({ id: interaction.user.id, tag: interaction.user.username, eliminated: false });
      const embed = EmbedBuilder.from(interaction.message.embeds[0]).setFields({
        name: `اللاعبون المنضمون (${session.players.length})`,
        value: session.players.map((p) => `<@${p.id}>`).join('\n')
      });
