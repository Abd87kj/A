...
async function generateBombWinnerImage(winner, bannerUrl = null) {
  ...
  // ── الخلفية ──
  // أولوية: بانر مخصص من إعدادات السيرفر، وإلا خلفية Løst الافتراضية، وإلا تدرج بديل
  let hasImageBg = false;
  if (bannerUrl) {
    try {
      const buf = await fetchImageBuffer(bannerUrl);
  ...
  // ── أفاتار الفائز - بالضبط داخل الدائرة الموجودة بتصميم الخلفية ──
  // مركز الدائرة ونصف قطرها محسوبين على أبعاد 700x300 بعد قص الصورة (cover-fit)
  const avR = 100;
  const avX = W / 2;   // 350
  const avY = 110;
