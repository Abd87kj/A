...
async function generateWheelImage(
  ...
  if (showNames) {
    for (let i = 0; i < n; i++) {
      ...
      const colorObj = COLORS[i % COLORS.length];
      // النص دائماً أبيض إلا لو الشريحة فاتحة جداً
      const fillColor = typeof colorObj === 'string' ? colorObj : (colorObj.outer || '#e0e0e0');
...
async function generateWinnerImage(winner, pot, bet) {
  const W = 600, H = 420;
  const canvas = createCanvas(W, H);
  const ctx = canvas.getContext('2d');
  ...
  ctx.textBaseline = 'middle';
  ctx.fillText('👑', avatarX, avatarY - avatarSize / 2 - 28);
  ...
  ctx.shadowBlur = 6;
  ctx.fillText(winner.displayName || winner.username || '؟', W / 2, avatarY + avatarSize / 2 + 18);
  ...
  ctx.shadowColor = '#FFD700';
  ctx.shadowBlur = 10;
  ctx.fillText(`💰 +${pot} نقطة`, W / 2, avatarY + avatarSize / 2 + 65);
  ctx.shadowBlur = 0;
