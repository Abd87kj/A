...
async function generateWheelImage(
  ...
  if (n > 0) {
    ...
    // ── رسم الأسماء منفصل عن دوران العجلة ──
    if (showNames) {
      for (let i = 0; i < n; i++) {
        const midAngle = (i * sliceAngle) + sliceAngle / 2 + rotationAngle - Math.PI / 2;
        ...
        ctx.save();
        ctx.translate(tx, ty);
        ctx.rotate(midAngle + Math.PI / 2); // يدير النص ليكون قابل للقراءة
