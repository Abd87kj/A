...
// ألوان داكنة وأنيقة (تستخدم فقط لو ما فيه ثيم/ألوان مخصصة)
...
async function generateWheelImage(
  ...
  for (let i = 0; i < n; i++) {
    ...
    const baseColor = typeof colorObj === 'string' ? colorObj : colorObj.outer;
    const neonColor = (typeof colorObj === 'object' && colorObj.neon) || '#ffffff';
    ...
    const g1x = cx + Math.cos(mid) * outerR * 0.25;
    const g1y = cy + Math.sin(mid) * outerR * 0.25;
    const g2x = cx + Math.cos(mid) * outerR;
    const g2y = cy + Math.sin(mid) * outerR;
    const sg = ctx.createLinearGradient(g1x, g1y, g2x, g2y);
    sg.addColorStop(0, lightenColor(baseColor, 35));
    sg.addColorStop(1, baseColor);
    ctx.fillStyle = sg;
    ctx.fill();

    if (styleConfig?.borderStyle === 'thin') {
      // مينيمال: حد رفيع بدون توهج
      ctx.strokeStyle = 'rgba(255,255,255,0.15)';
      ctx.lineWidth = 1;
      ctx.stroke();
    } else if (styleConfig?.borderStyle === 'gold') {
      // ملكي: حد ذهبي
      ctx.strokeStyle = '#e8cc60';
      ctx.lineWidth = 2;
      ctx.shadowColor = '#e8cc60';
      ctx.shadowBlur = 6;
      ctx.stroke();
      ctx.shadowBlur = 0;
    } else {
      // كلاسيكي/نيون: توهج بلون النيون الخاص بالثيم
      ctx.strokeStyle = neonColor;
      ctx.lineWidth = styleConfig?.borderStyle === 'neon' ? 2.5 : 2;
      ctx.shadowColor = neonColor;
      ctx.shadowBlur = styleConfig?.outerGlow ? 10 : 0;
      ctx.stroke();
      ctx.shadowBlur = 0;
    }
  }
