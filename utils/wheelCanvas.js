...
// bannerUrl: رابط اختياري من إعدادات السيرفر
async function generateBannerImage(bannerUrl = null) {
  const W = 700, H = 350;
  ...
  if (bannerUrl) {
    try {
      ...
      console.warn('⚠️ فشل تحميل بانر السيرفر:', e.message);
    }
  }

  if (!drawn) {
    for (const p of DEFAULT_BANNER_PATHS) {
      if (fs.existsSync(p)) {
        try {
...
// colors: مصفوفة ألوان مخصصة من إعدادات السيرفر (اختياري)
async function generateWheelImage(players, winnerIndex, rotationAngle = 0, colors = null) {
  ...
  return { buffer: canvas.toBuffer('image/png'), ctx };
}
...
function generateSmoothAngles(winnerIndex, playerCount, steps = 60) {
...
function getFrameDelays(steps = 60) {
  const delays = [];
  for (let i = 0; i < steps; i++) {
    const t = i / steps;
    if (t < 0.15)      delays.push(35);
    else if (t < 0.50) delays.push(40);
    else if (t < 0.70) delays.push(55);
    else if (t < 0.82) delays.push(80);
    else if (t < 0.90) delays.push(130);
    else if (t < 0.95) delays.push(220);
    else               delays.push(350);
  }
  return delays;
}

// يولّد GIF حقيقي وسلس لدوران العجلة (بدون تعديل رسائل متكرر)
async function generateSpinGif(players, winnerIndex, colors = null, steps = 60) {
  const SIZE = 500; // تصغير الحجم يخفف وزن GIF
  ...
  for (let i = 0; i < steps; i++) {
    ...
    const { ctx } = await generateWheelImageScaled(players, isLast ? winnerIndex : null, angles[i], colors, SIZE);
    encoder.setDelay(delays[i]);
    encoder.addFrame(ctx);
  }
  // ثبّت آخر فريم لثانيتين عشان يظهر الفوز بوضوح
  encoder.setDelay(2000);
  const { ctx: lastCtx } = await generateWheelImageScaled(players, winnerIndex, angles[steps - 1], colors, SIZE);
...
// نسخة من generateWheelImage بحجم مخصص (للـGIF) ترجع الكونتكست مباشرة
async function generateWheelImageScaled(players, winnerIndex, rotationAngle, colors, size) {
  const full = await generateWheelImage(players, winnerIndex, rotationAngle, colors);
  if (size === 700) return full;
  const canvas = createCanvas(size, size);
  const ctx = canvas.getContext('2d');
  const img = await loadImage(full.buffer);
