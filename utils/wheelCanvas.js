...
// عدد فريمات الدوران - موحّد ومُصدّر عشان نفس القيمة تُستخدم بملف الكوماند بدون تكرار/اختلاف بالأرقام
const SPIN_STEPS = 72;
...
async function generateWheelImage(players, winnerIndex, rotationAngle = 0, colors = null, size = 640) {
  ...
  if (n === 0) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      ctx.fillStyle = '#ffffff'; // أبيض ثابت لكل القطع - أوضح وأعلى تباين على أي خلفية
      ...
      // نقلل الحد الأقصى للحروف شوي عشان نعوّض مساحة رقم الترتيب المُضاف قبل الاسم
...
function generateSmoothAngles(winnerIndex, playerCount, steps = SPIN_STEPS) {
...
/**
 * مدة عرض كل فريم بالملي ثانية. قبل كذا كانت تتغيّر بخطوات مفاجئة (buckets) مع إن
 * الزاوية نفسها تتغيّر بانسيابية - هذا التضارب هو سبب إحساس "التقطيع" بالدوران.
 * الحل: نخليها قيمة ثابتة (baseDelay) خلال الدوران السريع المتزن، وفي آخر 25%
 * من الوقت (مرحلة التهدئة) نزيدها بشكل تدريجي وسلس (تربيعي، بلا أي قفزة) فقط.
 */
function getFrameDelays(steps = SPIN_STEPS, baseDelay = 55, maxTailMultiplier = 5) {
  const delays = [];
  for (let i = 0; i < steps; i++) {
    const t = i / steps;
    let multiplier = 1;
    if (t > 0.75) {
      const localT = (t - 0.75) / 0.25; // 0 -> 1 بشكل مستمر
      multiplier = 1 + (maxTailMultiplier - 1) * (localT * localT); // تربيعي = تدرّج سلس بدون قفزة
    }
 ...
 */
function getTotalGifDuration(steps = SPIN_STEPS, finalHold = 3000) {
